...
fn main() {
    ...
    let size1: size_t = 10 as size_t;
    let size2: size_t = 12 as size_t;
    let size3: size_t = 14 as size_t;
    ...
    //     print_content_all();
    // }
    ffi::my_create_content(size1, buf1.as_ptr() as *mut c_char, id1);
    ffi::my_create_content(size2, buf2.as_ptr() as *mut c_char, id2);
    ffi::my_create_content(size3, buf3.as_ptr() as *mut c_char, id3);
    ffi::my_print_content_all();
    ffi::my_free(id2);
    ffi::my_print_content_all();
    ffi::my_print_content(id1);
    ffi::my_free_all();
    ffi::my_print_content_all();
    // unsafe{ fin(pop) };
    ffi::my_fin(pop);
